...
class Analogy(Model):
    ...
    def embedding_def(self):
        ...
        self.ent1_embeddings = tf.get_variable(
                ...
                shape=[self.n_entities, self.hidden_size/2],
                initializer=tf.contrib.layers.xavier_initializer(uniform = True))

        self.rel1_embeddings = tf.get_variable(
                name="rel1_embeddings",
                shape=[self.n_relations, self.hidden_size/2],
                initializer=tf.contrib.layers.xavier_initializer(uniform = True))
    ...
    def predict(self, predict_h, predict_t, predict_r):
        ...
        return -tf.reduce_sum(self._calc_comp(predict_h_e1, predict_h_e2, predict_t_e1, predict_t_e2, predict_r_e1, predict_r_e2), 1, keep_dims = True) - tf.reduce_sum(self._calc_dist(predict_h_e, predict_t_e, predict_rel), 1, keep_dims = True)
